...
def analytic_damping_density(sigma, K, V):
    ###
    # sigma is also carrying grad
    ###
    Kx, Ky, Kz = K
    # if Kx.requires_grad : Kx.register_hook(lambda x : print("Kx: ", Kx.grad_fn,torch.isnan(x)))
    eps=2.2204e-16

    A = -(Kx**2 + Ky**2 + Kz**2)
    # if A.requires_grad : A.register_hook(lambda x : print("A: ", A.grad_fn,torch.any(torch.isnan(x))))
    B = 2 * sigma * Kz
    # if B.requires_grad : B.register_hook(lambda x : print("B: ", B.grad_fn,torch.any(torch.isnan(x))))
    C = Kx**2 + Ky**2 - sigma**2
    # if C.requires_grad : C.register_hook(lambda x : print("C: ", C.grad_fn,torch.any(torch.isnan(x))))
    tmpy = (B**2 - 4 * A * C)
    # remove negative numbers
    # tmpy[tmpy<0] = eps
    #delta = torch.Tensor(np.real([cmath.sqrt(it) for it in tmpy])) #torch.sqrt(B**2 - 4 * A * C)
    delta = torch.sqrt(torch.clamp(tmpy, min=torch.tensor(eps)) )
    # if delta.requires_grad : delta.register_hook(lambda x : print("delta: ", delta.grad_fn,torch.any(torch.isnan(x))))
    ...
    p1 = func_p(a1, b , c)
    p2 = func_p(a2, b , c)
    # if p0.requires_grad : p0.register_hook(lambda x : print("p0: ", p0.grad_fn,torch.any(torch.isnan(x))))
    ...
    #Fint = lambda p_up, p_low:(F(p_up) - F(p_low))
    Fint = lambda p_up, p_low:(func_F(p_up,A,B,C,delta) - func_F(p_low,A,B,C,delta))

    H0 = Fint(p0[1, :], p0[0, :])
    H1 = Fint(p1[1, :], p1[0, :])
    H2 = Fint(p2[1, :], p2[0, :])
    # if H1.requires_grad : H1.register_hook(lambda x : print("H1: ", H1.grad_fn,torch.any(torch.isnan(x))))
...
def func_F(phi, A, B, C, delta, eps=2.2204e-16):
    ...
    # a_sin[a_sin<-1] = -0.99999
    a_sin2 = torch.clamp(a_sin, min=torch.tensor(-0.99999), max=torch.tensor(0.99999))
    v2 = torch.asin(a_sin2)
    return (v1*v2)
