...
# Env maker
def envelope_generator(inp, filter_len=2047, gain=None, clip_=None, normalise=False, title_str = '', title_sup = '', display_plots=False, device='cpu'):
    eps=2.2204e-16
    # Amplify
    ...
    if display_plots:
        ...
        if device == 'cuda':
            ...
            llist = [inp.detach().cpu(), smoothed.detach().cpu(),in_db.detach().cpu(),normalised_c.detach().cpu(),clipped_db.detach().cpu()]
        else:
            llist = [inp, smoothed, clipped_db, normalised]
        #
